...
export const update = (table: string, select: string, columns: string[], returning = "*") => {
  const set = columns.map((column, index) => `${column} = $${index + 1}`).join(", ")
  return `UPDATE ${table} SET ${set} WHERE ${select || "id"} = $${columns.length + 1} RETURNING ${returning}`
}
